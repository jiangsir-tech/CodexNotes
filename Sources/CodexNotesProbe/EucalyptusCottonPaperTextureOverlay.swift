...
/// A deterministic, near-invisible pressed-pulp layer for the eucalyptus theme.
/// Tiny curved grains keep the surface tactile without resembling tracing fibers
/// or the North Sea theme's cold-pressed dimples and pigment edges.
struct EucalyptusCottonPaperTextureOverlay: View {
    private let cellSize: CGFloat = 156
    private let seed: UInt64 = 0x455543414C595054
    ...
    private func drawCell(
        ...
        for index in 0..<4 {
            ...
            let length = interpolate(4...13, unit(value >> 32))
            ...
            let control = CGPoint(
                x: (start.x + end.x) / 2 + interpolate(-1.2...1.2, unit(mix(value, 5))),
                y: (start.y + end.y) / 2 + interpolate(-1.2...1.2, unit(mix(value, 7)))
            )

            var grain = Path()
            grain.move(to: start)
            grain.addQuadCurve(to: end, control: control)
            context.stroke(
                grain,
                with: .color(
                    Color(red: 0.235, green: 0.392, blue: 0.337)
                        .opacity(Double(interpolate(0.005...0.010, unit(mix(value, 11)))))
                ),
        ...
        for index in 0..<2 {
            ...
            let diameter = interpolate(0.45...1.0, unit(value >> 32))
            context.fill(
                Path(
                    ellipseIn: CGRect(
                ...
                with: .color(
                    Color(red: 0.360, green: 0.490, blue: 0.440)
                        .opacity(Double(interpolate(0.005...0.009, unit(value >> 48))))
                )
            )
        }
    }
